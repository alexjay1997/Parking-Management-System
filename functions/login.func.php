...
$select_login_user = $conn_select_user_login->select_login_user($username,$Encrypt_password);
 //$query=mysqli_query($conn,"select * from tbl_users where username='$username' && password='$Encrypt_password'");

if($select_login_user->num_rows>0){
  $row =mysqli_fetch_array($select_login_user);
